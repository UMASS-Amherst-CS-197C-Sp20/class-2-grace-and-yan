...
// Given an array of doubles `entries` of size `length`, let's
// calculate some basic statistics (Check wikipedia if you don't
// remember how to do stddev).
...
// When scanf gets the wrong thing, it halts immediately. This
// function "clears" input by reading until the next newline or the
// end of the stream (EOF).
void ignore_input() {
  int c;
  while (1) {
    c = getchar();
